...
class Collector {
    ...
    handle(action, data) {
        return Promise.resolve(this.handleAction(action, data))
    }
    ...
    handleAction(action, data) {
        switch (action) {
            case 'createSnapshot':
                ...
                return this.newSnapshot(snapshotDate)

            case 'publishSnapshotItem':
                return this.acceptSnapshotItems(data.snapshot_id, data.items)

            case 'activateSnapshot':
                return this.activateSnapshot(data.snapshot_id)

            case 'diff':
                const diffDate = new Date(data.date);
                return this.newDiff(data.snapshot_id, diffDate)

            case 'publishDiffItem':
                return this.acceptDiffItems(data.diff_id, data.items)

            case 'activateDiff':
                return this.activateDiff(data.diff_id)
        }
    }
