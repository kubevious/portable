...
export class ReporterTarget {
  ...
  constructor(logger: ILogger, config: any) {
    ...
    this._jobDampener = new JobDampener<Snapshot>(
      ...
      this._processSnapshot.bind(this)
    );
  }
  ...
  private _processSnapshot(date: Date, snapshot: Snapshot): Promise<any> {
    this._logger.info(
      "[_processSnapshot] date: %s, item count: %s",
      date.toISOString(),
      snapshot.count
    );
  ...
  private _reportSnapshot(snapshot: Snapshot): Promise<any> {
    ...
    var snapshotReporter = new SnapshotReporter(
      this,
      this._snapshotLogger,
      snapshot,
      this._latestSnapshot,
      this._latestSnapshotId
    );
    return snapshotReporter.run().then(() => {
      ...
      if (snapshotReporter.isReported) {
        ...
        this._logger.info(
          "[_reportSnapshot] Completed. Latest Snapshot Id:",
          this._latestSnapshotId
        );
      } else {
  ...
  private _retrySnapshotReport(snapshot: Snapshot): Promise<any> {
    return Promise.timeout(3000).then(() => this._reportSnapshot(snapshot));
  }

  request(url: string, data: any) {
    let action = new RetryableAction(
      this.logger,
      () => {
  ...
  private _rawRequest(url: string, data: any) {
    ...
    return this._prepareRequest()
      ...
      .catch((reason) => {
        if (reason.response) {
          this.logger.error(
            "[request] URL: %s, RESPONSE STATUS: %s",
            url,
          ...
          if (reason.response.status == 413) {
            var size = _.get(
              reason,
              "request._redirectable._requestBodyLength"
            );
            this.logger.warn(
              "[request] Request too big. Ingoring. URL: %s, Size: %s bytes",
              url,
              size
            );
            return {};
  ...
  private _getApiKey() {
    ...
    return fs
      .readFile(this._config.keyPath, { encoding: "utf8" })
      .then((data: any) => {
