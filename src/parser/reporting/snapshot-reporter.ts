...
export class SnapshotReporter {
  ...
  private _createSnapshot(): Promise<any> {
    ...
    return this._request("/snapshot", body).then((result: any) => {
  ...
  private _publishSnapshotItems(): Promise<any> | void {
    ...
    return Promise.serial(
      reportableItems,
      this._publishSnapshotItem.bind(this)
    );
  }
  ...
  private _publishSnapshotItem(item: any): Promise<any> | void {
    ...
    return this._request("/snapshot/items", data).then((result: any) => {
  ...
  private _activateSnapshot(): Promise<any> | void {
    ...
    return this._request("/snapshot/activate", data).then((result: any) => {
  ...
  private _createDiff(): Promise<any> | void {
    ...
    return this._request("/diff", body).then((result: any) => {
  ...
  private _publishDiffItem(item: any): Promise<any> | void {
    ...
    return this._request("/diff/items", data).then((result: any) => {
  ...
  private _activateDiff(): Promise<any> | void {
    ...
    return this._request("/diff/activate", data).then((result: any) => {
      ...
      if (result.new_snapshot) {
        ...
        this._isReported = true;
        this.logger.info(
          "[_activateDiff] activated: %s. new snapshot id: %s.",
          this._diffId,
          this._snapshotId
        );
      }
