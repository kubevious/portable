...
export default LogicParser()
  ...
  .handler(({ scope, item, logger, helpers }) => {
    ...
    let appsByConsumptionDict: Record<
      string,
      {
        dn: string;
        max: number;
    ...
    for (let app of item.getChildrenByKind("app")) {
      ...
      if (appResourcesProps) {
        let appResources = <Record<string, PropertyValueWithUnit>>(
          appResourcesProps.config
        );
        for (let metric of helpers.resources.METRICS) {
      ...
      if (appUsedResourcesProps) {
        let appUsedResources: Record<string, PropertyValueWithUnit> =
          appUsedResourcesProps.config;
        for (let metric of helpers.resources.METRICS) {
    ...
    for (let appConsumption of _.values(appsByConsumptionDict)) {
      ...
      appConsumption.max = _.max(
        helpers.resources.METRICS.map(
          (metric) => appConsumption.metrics[metric]
        )
      )!;
    }

    appsByConsumptionTable.rows = _.orderBy(
      _.values(appsByConsumptionDict),
      (x) => x.max,
      "desc"
    ).map((x) => {
