...
export class LogicProcessor {
  ...
  private _extractProcessors(location: string) {
    ...
    for (var processor of processors) {
      this._logger.info(
        "[_extractProcessors] HANDLER: %s -> %s, target:",
        processor.order,
        processor.name
      );
  ...
  private _loadProcessor(
    name: string,
    location: string,
    processors: BaseParserExecutor[]
  ) {
  ...
  process() {
    return this._context.tracker
      .scope("Logic::process", (tracker: any) => {
  ...
  _report(scope: LogicScope, tracker: any) {
    return tracker.scope("report", () => {
      return this._context.facadeRegistry.acceptLogicItems(
        scope.extractItems()
      );
    });
  }
  ...
  private _processParser(scope: LogicScope, handlerInfo: BaseParserExecutor) {
    this._logger.debug(
      "[_processParser] Handler: %s -> %s, target: %s :: ",
      handlerInfo.order,
      handlerInfo.name
    );
