...
export default ScopeParser()
  ...
  .handler(({ scope, itemScope, namespaceScope, createK8sItem }) => {
    ...
    for (var appScope of appScopes) {
      ...
      processRules(k8sNetworkPolicy, "Ingress", "spec.ingress", "from");

      processRules(k8sNetworkPolicy, "Egress", "spec.egress", "to");
    }
          ...
          if (rules) {
            for (var rule of rules) {
              ...
              if (ipBlock) {
                ...
                let namespaceSelectorLabels = _.get(
                  rule,
                  "namespaceSelector.matchLabels"
                );
                if (namespaceSelectorLabels) {
                  targetNamespaceScopes = scope.findNamespaceScopesByLabels(
                    namespaceSelectorLabels
                  );
                }

                for (let targetNamespaceScope of targetNamespaceScopes) {
                  var fromAppScopes = targetNamespaceScope!.findAppScopesByLabels(
                    podSelectorLabels
                  );
