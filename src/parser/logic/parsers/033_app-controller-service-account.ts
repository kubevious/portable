...
/*** SOURCE: ../parser.git/src/logic/parsers/033_app-controller-service-account.ts                        ***/
...
export default LogicParser()
  ...
  .handler(({ item, namespaceScope }) => {
    ...
    if (name) {
      var serviceAccountScope = namespaceScope.items.get(
        "ServiceAccount",
        name
      );
      if (serviceAccountScope) {
        ...
        app.addAlert(
          "Missing",
          "error",
          "Service account " + name + " is not found."
        );
      }
    } else {
