...
/*** SOURCE: ../parser.git/src/logic/parsers/040_service.ts                                               ***/
      ...
      if (
        ...
        if (appSelector) {
          ...
          for (let appScope of appScopes) {
            ...
            let serviceCount = appItem
              .getChildrenByKind("service")
              .filter(() => serviceType == serviceItemName).length;
            if (serviceCount != 0) {
            ...
            if (portsConfig) {
              for (let portConfig of portsConfig) {
                ...
                if (appPortInfo) {
                  createService(appPortInfo.portItem, {
                  ...
                  createAlert(
                    "Port-" + appPort,
                    "warn",
                    "Missing port " + appPort + " definition."
                  );
                }
              }
            }
          ...
          if (appScopes.length == 0) {
            createAlert(
              "MissingApp",
              "error",
              "Could not find apps matching selector."
            );
          } else if (appScopes.length > 1) {
            createAlert(
              "MultipleApps",
              "warn",
              "More than one apps matched selector."
            );
          }
        }
      }
