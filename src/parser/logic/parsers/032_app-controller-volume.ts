  ...
  .handler(({ scope, item, createItem, createAlert, namespaceScope }) => {
    ...
    if (volumesConfig.length > 0) {
      ...
      for (var volumeConfig of volumesConfig) {
        processVolumeConfig(volumes, volumeConfig, false);
      }
    ...
    function processContainerItem(container: LogicItem) {
      ...
      for (var volumeRefConfig of volumeMounts) {
        ...
        if (volumeConfig) {
          var volumeItem = processVolumeConfig(container, volumeConfig, true);
    ...
    function processVolumeConfig(
      ...
      if (volumeConfig.configMap) {
        findAndProcessConfigMap(
          volume,
          volumeConfig.configMap.name,
          markUsedBy,
          volumeConfig.configMap.optional
        );
      }

      if (volumeConfig.secret) {
        findAndProcessSecret(
          volume,
          volumeConfig.secret.secretName,
          markUsedBy
        );
      }
    ...
    function findAndProcessConfigMap(
      parent: LogicItem,
      name: string,
      markUsedBy: boolean,
      isOptional: boolean
